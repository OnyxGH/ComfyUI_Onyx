...
class AbstractDiffusion:
    ...
    @grid_bbox
    def get_grid_bbox(
        self, tile_w: int, tile_h: int, overlap: int, tile_bs: int, w: int, h: int, device: torch.device, get_tile_weights: Callable = lambda: 1.0
    ) -> List[List[BBox]]:
    ...
    def process_controlnet(self, x_noisy, c_in: dict, cond_or_uncond: List, bboxes, batch_size: int, batch_id: int, shifts=None, shift_condition=None):
        ...
        if not hasattr(control, "cond_hint"):
            # Newer sampler paths may pass already-evaluated control dicts.
            # Tiled control processing only works with ControlNet-like objects.
            return False
            ...
            if self.refresh or control.cond_hint is None or not isinstance(self.control_params[tuple_key][param_id][batch_id], Tensor):
                ...
                if isinstance(control, T2IAdapter):
                    ...
                    if getattr(control, "sub_idxs", None) is not None and control.cond_hint_original.shape[0] >= control.full_latent_length:
                        cns = common_upscale(control.cond_hint_original[control.sub_idxs], PW, PH, control.upscale_algorithm, "center").to(
                ...
                for bbox in bboxes:
                    y0 = int(bbox[1] * int_cf)
                    y1 = int(bbox[3] * int_cf)
                    x0 = int(bbox[0] * int_cf)
                    x1 = int(bbox[2] * int_cf)
                    # clamp
                    y0 = max(0, min(y0, Hc))
                    y1 = max(y0, min(y1, Hc))
                    x0 = max(0, min(x0, Wc))
                ...
                if not cns_slices:
                    ...
                    h_fallback = max(1, h_fallback)
                    w_fallback = max(1, w_fallback)
    ...
    def __call__(self, model_function: BaseModel.apply_model, args: dict):
        ...
        if self.draw_background:
            for batch_id, bboxes in enumerate(self.batched_bboxes):
                ...
                if "control" in c_in:
                    control_ready = self.process_controlnet(x_tile, c_in, cond_or_uncond, bboxes, N, batch_id)
                    if control_ready and hasattr(c_in["control"], "get_control_orig"):
                        c_tile["control"] = c_in["control"].get_control_orig(x_tile, t_tile, c_tile, len(cond_or_uncond), c_in["transformer_options"])
                    else:
                        c_tile["control"] = c_in["control"]
...
class SpotDiffusion(AbstractDiffusion):
    ...
    def __call__(self, model_function: BaseModel.apply_model, args: dict):
        ...
        if self.uniform_distribution is not None and (
            self.sigmas is not store.sigmas or getattr(self, "_spot_shift_method", None) != shift_method or getattr(self, "_spot_seed", None) != seed
        ):
            self.uniform_distribution = None
        ...
        if self.uniform_distribution is None:
            ...
            self._spot_shift_method = shift_method
            self._spot_seed = seed
        ...
        if self.draw_background:
            for batch_id, bboxes in enumerate(self.batched_bboxes):
                ...
                # self.switch_controlnet_tensors(batch_id, N, len(bboxes))
                if "control" in c_in:
                    control_ready = self.process_controlnet(x_tile, c_in, cond_or_uncond, bboxes, N, batch_id, (sh_h, sh_w), condition)
                    if control_ready and hasattr(c_in["control"], "get_control_orig"):
                        c_tile["control"] = c_in["control"].get_control_orig(x_tile, t_tile, c_tile, len(cond_or_uncond), c_in["transformer_options"])
                    else:
                        c_tile["control"] = c_in["control"]
            ...
            for batch_id, bboxes in enumerate(self.batched_bboxes):  # batch_id is the `Latent tile batch size`
                ...
                for i, bbox in enumerate(bboxes):
                    ...
                    dbg(
                        ...
                        i,
                        "slice_in.shape=",
                        tuple(slice_in.shape),
                        "rf_slice.shape=",
                        tuple(rf_slice.shape),
                        "tile_weights.shape=",
                        tuple(self.tile_weights.shape),
                        "final_w.shape=",
                        tuple(w.shape),
                    )
                    try:
